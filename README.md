# Customer Call List - Data Cleaning (Pandas)

**Project Overview:** This project focuses on **Data Cleaning** using Python and Pandas. 
The raw dataset (`Customer Call List.xlsx`) from kaggle contained real-world CRM data issues such as duplicate entries, formatting inconsistencies, missing values, and non-compliant records.

The goal was to transform dirty data into a structured, production-ready dataset.

---

## Tech Stack
* **Language:** Python
* **Library:** Pandas
* **Environment:** Jupyter Notebook

---

## Key Cleaning Steps
1. Removing Duplicates & Irrelevant Data
2. Text Cleaning & Standardization
3. Compliance Filtering & Missing Values
